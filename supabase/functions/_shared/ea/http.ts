 ...
 * BASE_URL = /api/fc seulement. Ne pas poser EA_FC_TITLE=fc27 ici.
 */
...
export function getEaFcBaseUrl(): string {
  const source = (readEnv("EA_FC_BASE_URL") ?? DEFAULT_EA_FC_BASE_URL).trim().replace(/\/+$/, "");
  if (!/^https:\/\//i.test(source)) return DEFAULT_EA_FC_BASE_URL;
  if (/\/api\/fifa(\/|$)/i.test(source)) return DEFAULT_EA_FC_BASE_URL;
  if (!/\/api\/fc(\d+)?$/i.test(source)) return DEFAULT_EA_FC_BASE_URL;
  return source;
}
