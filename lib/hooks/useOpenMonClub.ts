...
import { CLUB_MATCHMAKING_HREF, splitMemberships } from "@/lib/monClubNav";

/**
 * Même destination que le menu « Mon club » :
 * OWNER/MANAGER → feuille Matchmaking club ; MEMBER → ClubHome ; aucun → /clubs.
 */
export function useOpenMonClub() {
  ...
  const openMonClub = useCallback(() => {
    ...
    if (managed.length > 0) {
      ...
      const goFeuille = () => router.push(CLUB_MATCHMAKING_HREF as Href);
      if (mode !== "CLUB") {
        setMode("CLUB");
