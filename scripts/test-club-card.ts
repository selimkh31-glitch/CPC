...
test("spine UX — Recrutement invite ; Club pas un 2e LIVE ; Card a Lier mon club", () => {
  ...
  assert.true(liveClub.includes("ClubLiveRecruit"), "club LIVE is recruit");
  ...
  assert.true(recruit.includes("ClubDiscoveryToggle"), "discovery toggle on matchmaking");
  assert.true(feuille.includes("FormationPitch"), "pitch on Match");
  assert.false(feuille.includes("ClubDiscoveryToggle"), "feuille is not recruit");
  assert.false(recruit.includes("FormationPitch"), "recruit is not feuille");
