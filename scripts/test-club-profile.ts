...
test("pricingScreenCopy — sans RevenueCat : pas d'euro, features EA pas live", () => {
  ...
  for (const fake of ["Saison CPC", "Badges de saison", "Stats EA liées", "Carte animée premium"]) {
    if (copy.liveFeatures.some((f) => f.includes(fake)) || live.includes(fake)) {
      throw new Error(`${fake} ne doit pas être listé comme feature live.`);
    }
  }
